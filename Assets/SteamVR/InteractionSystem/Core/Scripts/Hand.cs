...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        public struct AttachedObject
        {
            ...
            public Vector3 pseudoSourcePosition;
            public Quaternion pseudoSourceRotation;
        ...
        public void AttachObject(GameObject objectToAttach, GrabTypes grabbedWithType, AttachmentFlags flags = defaultAttachmentFlags, Transform attachmentOffset = null)
        {
            ...
            if (attachedObject.interactable != null)
            {
                ...
                //Pseudo-Hapticsにチェックがついていたら
                if (attachedObject.interactable.pseudoHaptics)
                {
                    //アタッチされたオブジェクトの位置を記録
                    attachedObject.pseudoSourcePosition = attachedObject.attachedObject.transform.position;
                    //アタッチされたオブジェクトの回転を記録
                    attachedObject.pseudoSourceRotation = attachedObject.attachedObject.transform.rotation;
                }
            ...
            if (attachedObject.interactable != null && attachedObject.interactable.pseudoHaptics)
            {
                attachedObject.attachedObject.transform.position = attachedObject.pseudoSourcePosition;
                attachedObject.attachedObject.transform.rotation = attachedObject.pseudoSourceRotation;
            }
        ...
        {
            // 現在アタッチされているオブジェクトがあるかどうかを確認し、必要に応じて制御を行います
            if (currentAttachedObject != null)
            {
                // 現在アタッチされているオブジェクトの情報を取得します
                AttachedObject attachedInfo = currentAttachedObjectInfo.Value;
                // アタッチされているオブジェクトが有効である場合、その動作を制御します
                if (attachedInfo.attachedObject != null)
                {
                    // オブジェクトが速度で移動している場合の処理
                    if (attachedInfo.HasAttachFlag(AttachmentFlags.VelocityMovement))
                    {
                        // アタッチされたオブジェクトの速度を更新します
                        if (attachedInfo.interactable.attachEaseIn == false || attachedInfo.interactable.pseudoHaptics == false || attachedInfo.interactable.snapAttachEaseInCompleted)
                    ...
                    if (attachedInfo.interactable.attachEaseIn)
                    {
                        ...
                        if (t < 1.0f)
                        {
                            // オブジェクトの速度をリセットし、イーズインの進行度に基づいて移動と回転を補間します
                            if (attachedInfo.HasAttachFlag(AttachmentFlags.VelocityMovement))
                            {
                    ...
                    //Pseudo-Hapticsの実装
                    if (attachedInfo.interactable.pseudoHaptics)
                    {
                        if (attachedInfo.HasAttachFlag(AttachmentFlags.VelocityMovement))
                        {
                            attachedInfo.attachedRigidbody.velocity = Vector3.zero;
                        ...
                        attachedInfo.attachedObject.transform.position = Vector3.LerpUnclamped(attachedInfo.pseudoSourcePosition, TargetItemPosition(attachedInfo), attachedInfo.interactable.pseudoHapticsGain);
                        attachedInfo.attachedObject.transform.rotation = Quaternion.LerpUnclamped(attachedInfo.pseudoSourceRotation, TargetItemRotation(attachedInfo), 1);
                    }
                }
            }
        }
