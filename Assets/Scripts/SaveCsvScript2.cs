...
public class SaveCsvScript2 : MonoBehaviour
{
    ...
    void Start()
    {
        sw = new StreamWriter(@"SaveExperimentData2.csv", true, Encoding.GetEncoding("Shift_JIS"));
        string[] s1 = { "セット数", "1回目","2回目", "ref_or_com", "より重かった方", "その数値" };
        string s2 = string.Join(",", s1);
        sw.WriteLine(s2);

        swB = new StreamWriter(@"SaveExperimentAnalyzeData2.csv", true, Encoding.GetEncoding("Shift_JIS"));
    ...
    public void SaveDataEx2(string txt1, string txt2, string txt3, string txt4, string txt5, string txt6)
    {
